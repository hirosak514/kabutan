...
株探（kabutan.jp）の「銘柄探検」ページ（例: ゴールデンクロス銘柄一覧）から
銘柄リストを取得し、各銘柄についてGemini APIで企業概要・決算・PER/PBR/ROE・
配当利回り・アナリスト目標株価を要約し、日足/週足/月足のローソク足チャートと
合わせて一覧表示するStreamlitアプリです。
...
1. サイドバーの「対象URL」に株探の銘柄探検ページURLを入力（デフォルトは
   ゴールデンクロス銘柄一覧 `https://kabutan.jp/tansaku/?mode=2_0870`）
2. 「🔄 更新」ボタンを押すと、1〜2ページ目の銘柄コード・銘柄名を取得
3. 「🔍 分析」ボタンを押すと、各銘柄についてGeminiが下記5項目を要約し、
