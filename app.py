...
def parse_company_list(html: str):
    ...
    # ---- 要調整 ----
    実際のテーブル構造に合わせてセレクタを調整してください。
    想定: <a href="/stock/?code=XXXX">銘柄名</a> のようなリンクが
    テーブル内に複数存在する。
    """
    ...
    # 銘柄コードへのリンクを総当たりで探す（code=XXXX を含むaタグ）
    # 新形式の英字混じりコード（例: 143A）にも対応
    for a in soup.find_all("a", href=True):
        href = a["href"]
        m = re.search(r"code=([0-9][0-9A-Z]{3})", href)
        if not m:
            continue
        code = m.group(1)
